...
\begin{figure}[t]
  ...
  \caption{\textbf{Measuring Platonic convergence in the J-space.}
    ...
    alignment and $54.1\%$ of full-activation alignment. When applying the same
    J-space measurement with a random Gaussian dictionary rather than a J-lens,
    it is found that this alignment retains $34.7\%$
    (Table~\ref{tab:surrogate}).
    \textbf{(c)} In regards to language, J-space alignment rises with
    competence at a rate that is approximately one third the rate that full
    activation alignment does. Across modalities, J-space alignment and full
...
Our three experiments (Figure~\ref{fig:teaser}) compare 1) J-space against full alignment for text models (including the content-ablated motivating experiment); 2) text models against vision encoders, which have no J-space of their own; and 3) the J-lens readouts of text models, where outputs are vocabulary tokens and no machinery is needed. For each case, we calculate convergence as the correlation between alignment and competence. The third experiment also compares J-lens overlap with logit lens overlap~\citep{nostalgebraist2020logitlens} as a control.
...
Platonic convergence is also contested. \citet{groger2026revisiting} suggest model alignment is inflated, and \citet{bangachev2026representationalignmentrestslinear} partly attribute alignment to word frequency. It is well-established that alignment measures capture non-semantic structure. We instead examine how much of the competence--convergence relationship is semantic using the J-space. To our knowledge, the J-space has not yet been used to examine convergence.
...
Within-language, we find that there is a split along two metrics. The metrics which compare \emph{local neighbourhood structure} (e.g. $\kappa$-NN) follow our main paper's results, and the metrics which compare \emph{global subspace geometry}, i.e. how variance is distributed, do not. As well, the competence--alignment correlations for neighbourhood-based metrics all reach significance with $p<0.02$, but the correlations for global subspace metrics are not significant (the smallest is $p=0.267$).

\paragraph{Global subspace metrics are insensitive to semantic content}
We repeat the content-ablation experiment of Section~\ref{sec:textresults}, again only varying the metric. Results are shown in Table~\ref{tab:metriccontent}. Under the three global metrics, we find that full-space alignment is essentially unchanged by the ablation: each metric retains at least $96.9\%$. Notably, ablating semantic content nominally increased these metrics' alignment as shown by CKA and unbiased CKA both measuring retention values above $100\%$. For J-space ablation, the same metrics retain $88.1\%$, $85.4\%$, and $77.6\%$. In other words, under a quarter of J-space alignment is attributable to semantic content. For neighbourhood-based measures, the opposite is true. Thus, since this paper seeks to measure semantically-significant convergence, global subspace metrics are not appropriate.
