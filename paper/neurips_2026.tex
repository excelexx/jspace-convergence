...
% Note. For the workshop paper template, both \title{} and \workshoptitle{} are required, with the former indicating the paper title shown in the title and the latter indicating the workshop title displayed in the footnote. 
\hypersetup{pdftitle={How Much of Platonic Convergence is Semantic? Measuring Alignment in the Jacobian Subspace}}
\title{How Much of Platonic Convergence is Semantic? Measuring Alignment in the Jacobian Subspace}
...
\author{%
% Author block withheld for double-blind submission; restore for camera-ready.
%  Alexander Zhang \\
...
% LaTeX centres float-only pages by default (\@fptop = 0pt plus 1fil).
% Pin it to 0pt so a table that lands on its own page sits at the top.
\makeatletter
\setlength{\@fptop}{0pt}
\makeatother
...
\begin{abstract}
  The Platonic Representation Hypothesis~\citep{huh2024platonic} asserts that neural networks converge towards a shared representation of reality as they become more competent. Current evidence for this hypothesis comes from full activation space alignment, which mixes semantic content with superficial regularities like document length and structure. We instead measure alignment in the Jacobian space (J-space), the privileged subspace induced by the Jacobian lens (J-lens)~\citep{gurnee2026verbalizable}, and find that restricting the comparison to this subspace makes alignment more sensitive to semantic content, though a control attributes only part of this effect to the J-space itself.
  
  Measuring alignment in the J-space, we still find that models converge: the text--text Spearman correlation between competence and alignment is $\rho = +0.56$, $p = 0.0162$, and the text--vision correlation is $\rho = +0.97$, $p < 10^{-4}$. Within language, though, the J-space converges much more slowly than the full activation space, so part of the apparent link between competence and convergence is carried by superficial structure. Consistent with this, the gap between J-space and full convergence disappears across modalities, where no such structure is shared.
  
  This convergence is also legible: using the J-lens at matched layer depth, independently trained models name 5.1 of 25 shared vocabulary words against 2.0 for the plain logit lens. J-lens overlap rises with competence ($\rho = +0.74$, $p = 0.0020$). Overall, full-space alignment is partially confounded by superficial structure, and restricting alignment comparison to the J-space can help isolate semantically meaningful convergence. Code available at \url{https://anonymous.4open.science/r/jspace-convergence-anon/}.
\end{abstract}
\section{Introduction}
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{01_teaser.pdf}
  \caption{\textbf{Measuring Platonic convergence in the J-space.}
    \textbf{(a)} The J-lens splits a pooled activation into a J-space component
    and a non-J remainder. The document shown above is the
    example found in Section~\ref{subsec:texttextsetup}. The list of words is illustrative.
    \textbf{(b)} Randomising content words retains $26.3\%$ of J-space
    alignment and $54.1\%$ of full-activation alignment. Under the same measurement with a random Gaussian dictionary rather than a J-lens dictionary, alignment retains $34.7\%$ (Table~\ref{tab:surrogate}).
    \textbf{(c)} Within language, J-space alignment rises with
    competence at approximately one third the rate of full alignment. Across modalities, J-space alignment and full
    activation alignment converge at around equal rates
    (Figure~\ref{fig:lobf-components}).}
  \label{fig:teaser}
\end{figure}
...
representation spaces.'' We use two terms throughout this paper: a model pair's \emph{alignment} is how similar the representations of the two models are, and \emph{convergence} is the correlation between alignment and competence, across model pairs. The Platonic hypothesis claims this correlation is positive. 

To our knowledge, alignment has thus far been computed over full representational spaces.\footnote{We say ``full activation space'' rather than ``full representational space'', since every quantity we compare is a mean-pooled residual-stream vector at a given layer.} We show this method conflates semantic content with confounding surface-level similarities like structure, length, and formatting.

Recently, \citet{gurnee2026verbalizable} introduced a new interpretability
technique, the Jacobian lens (J-lens), which recovers the concepts a language model is poised to verbalise at a given layer. The J-lens induces the J-space, a privileged subspace of internal representations available for report and reasoning. It is semantically significant, acting as a ``global workspace'' for the model, and privileged, capturing a median of only 6--7\% of a concept vector's variance~\citep{gurnee2026verbalizable}.

Thus, we hypothesise that restricting the comparison to J-spaces will make alignment more sensitive to semantic content than the full-activation comparison, in the sense that J-space alignment will drop more sharply when semantic content is removed (Figure~\ref{fig:teaser}). 
...
We use the J-space to re-examine the Platonic Representation Hypothesis's central claim, and find that models do indeed converge under this subspace-restricted comparison.

Our three experiments compare 1) J-space against full alignment for text models (including the content-ablated motivating experiment); 2) text models against vision encoders, which have no J-space of their own; and 3) the J-lens readouts of text models, where outputs are vocabulary tokens and no additional alignment machinery is needed. For each case, we calculate convergence over model pairs. The third experiment also compares J-lens overlap with logit lens overlap~\citep{nostalgebraist2020logitlens} as a control.
...
Platonic convergence is also contested. \citet{groger2026revisiting} suggest model alignment is inflated, and \citet{bangachev2026representationalignmentrestslinear} partly attribute alignment to word frequency. It is well-established that alignment measures capture non-semantic structure. We instead examine how much of the competence--convergence relationship is semantic by comparing the J-spaces of models. To our knowledge, the J-space has not yet been used to examine convergence.
...
\item We find that part of the existing competence--convergence relationship rests on structure that is shared, but not semantic. Indeed, within language, J-space alignment rises with competence at roughly a third the rate of the full activation space, while across modalities (where no such shared structure exists) the two rates are indistinguishable. 
...
$\ell \in \{0,\dots,L_A-1\}$ and write $\lambda = \ell/(L_A-1) \in [0,1]$ for the
...
\subsection{Text--text alignment and motivating experiment}\label{subsec:texttextsetup}
...
Following the procedure described in the previous section, we compute alignment for the J-space, the non-J remainder, and the full activation space
over the original corpus. We then repeat the measurement on a content-ablated
variant of the same 1,000 documents. We preserve the sentence structure, replacing each non-stopword alphabetic token (i.e.\ each content word) with another drawn from the same log-frequency bin. For example,
...
When we randomise all content words, the full alignment retains a mean of 54.1\% of its original value, while J-space alignment retains only 26.3\% (Table~\ref{tab:surrogate}). To test how much of this drop comes from the sparse coding itself, we repeat the ablation experiment with a randomised Gaussian dictionary instead of the J-space dictionary. Under this control, retention is 34.7\%, meaning sparse coding accounts for 69.7\% of the drop between full and J-space retention. However, the remaining third is still attributable to the J-space itself. 
...
Mean alignment for the J-space ($0.420$), the non-J remainder ($0.506$), and the full activation space ($0.558$) is between $42.0\times$ and $55.7\times$ the chance level.

For each model pair, we compare alignment and the mean of their competences. We find a positive correlation across the J-space (Spearman correlation $\rho = +0.56$, $p = 0.0162$), full activation ($\rho = +0.77$, $p = 0.0008$), and remainder ($\rho = +0.87$, $p < 0.0001$) (Figure~\ref{fig:lobf-components}(a)). Since pairs are not independent, $p$-values come from permuting competence across models while holding alignment values fixed.
...
Alignment between text models and vision encoders for each component is between $6.8\times$ and $7.4\times$ the chance level. It correlates strongly and
...
When we replace the J-lens dictionary with random unembedding rows, J-space alignment exceeds the strongest of five seeded draws in all 55 pairs (mean margin $+0.167$). This shows that J-space alignment captures information that random vocabulary directions do not. The null dictionaries use a random subset of 20,000 unembedding rows rather than the full dictionary due to computational constraints.
...
The J-lens is fitted on ordinary text, so the reduced alignment on the content-ablated documents in the first experiment may result from the J-lens simply working less well on such documents. We find that degrading the lens cannot reproduce the effect of removing semantic meaning (Appendix~\ref{app:lensdegrade}).
...
remainder is close enough to the full activation that the two behave similarly.
...
We would like to strengthen our key claim about the J-space's semantic sensitivity with more tests on larger and more varied model types.
...
We would also welcome a theoretical explanation for our findings.
...
each vision encoder. Each encoder has a different alignment level, which is why we consider correlations within encoders rather than pooling across all 44 pairs. The relationship holds within every encoder, including MAE,
...
The main paper measures competence with HellaSwag exclusively. Holding alignment values fixed, we vary the competence axis and derive similar results, shown in Table~\ref{tab:competence} and Table~\ref{tab:crossaxes}.
...
We follow \citet{huh2024platonic} in using this competence measure for the cross-modal comparison only; we note that it gives a within-language J-space correlation of $\rho = +0.35$ ($p = 0.185$), which is not significant.
...
We construct an instrument-only null by reducing the number of dictionary directions on the original corpus until the J-space component accounts for the same fraction of the activation's squared norm as it does on content-ablated text. This gives a J-space that is degraded but computed on documents with intact meaning. Alignment between model pairs with the degraded J-space is $0.2924$ against $0.1099$ for the content-ablated text. Even a
...
Thus, lens degradation cannot reproduce the effect of removing semantic meaning.
...
We note that within language, regardless of metric, full alignment is always greater than J-space alignment. Cross-modally the two are near-indistinguishable, with J-space marginally ahead for six of the eight metrics, while the cross-modal correlation result is similarly strong ($+0.94 \leq \rho \leq +0.97$).

Within-language, metrics split into two groups. Metrics which compare \emph{local neighbourhood structure} (e.g. $\kappa$-NN) follow our main paper's results, and metrics which compare \emph{global subspace geometry}, i.e. how variance is distributed, do not. Furthermore, the competence--alignment correlations for neighbourhood-based metrics all reach significance with $p<0.02$, but the correlations for global subspace metrics are not significant (the smallest is $p=0.267$).

\paragraph{Global subspace metrics are insensitive to semantic content}
We repeat the content-ablation experiment of Section~\ref{sec:textresults}, again only varying the metric. Results are shown in Table~\ref{tab:metriccontent}. Under the three global metrics, we find that full-space alignment is essentially unchanged by the ablation: each metric retains at least $96.9\%$. Notably, ablating semantic content nominally increases these metrics' alignment as shown by CKA and unbiased CKA both measuring retention values above $100\%$. For J-space ablation, the same metrics retain $88.1\%$, $85.4\%$, and $77.6\%$. In other words, under a quarter of J-space alignment is attributable to semantic content. For neighbourhood-based measures, the opposite is true. Thus, since this paper seeks to measure semantically significant convergence, global subspace metrics are not appropriate.
...
The percentage of the drop in retention explained by the J-space matches our main results only for the neighbour-identity metrics (mutual $\kappa$-NN, LCS, and edit-distance). We note that the global subspace metrics carry little weight here, as they capture very limited semantic content.  
\begin{table}
  \centering
  \small
