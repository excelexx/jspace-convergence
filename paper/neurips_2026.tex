...
\begin{abstract}
  ...
  This convergence is also legible: Using the J-lens at matched layer depth, independently trained models name 5.0 of 25 shared vocabulary items above a position-shuffled floor against 1.9 for the plain logit lens. J-lens overlap converges with competence similarly to the kernel measure, at $\rho = 0.73$, $p=0.0021$.
\end{abstract}
...
We use the J-space to re-examine the Platonic Representation Hypothesis's central claim, and find find that models do indeed converge under this stricter measure.
...
positions. Any difference in tokenizer efficiency is quite marginal, from
$3.79$ to $4.19$ characters per token. Measured over the corpus, the least efficient tokenizer still ingests 94.8\% as many characters per document as the most efficient. Neighbourhoods are taken at $\kappa=10$.
...
\]
Since the 11 models do not share a tokenizer, we restrict the J-lens to the 31,548 single-token strings in all 11 vocabularies. We also index positions by character rather than by token, evaluating at 6,000 character positions that fall at a token-end word boundary in all 11 tokenizers.
...
$\mathrm{softmax}(W_U^A\,\mathrm{norm}^A(h))$. As in the first experiment, we then relate the resulting overlap to the mean HellaSwag accuracy of the pair to reevaluate the convergence trend.
...
Mean alignment for the J-space ($0.420$), the remainder ($0.506$), and the full activation ($0.558$) is between $42.0\times$ and $55.7\times$ chance levels.
...
positively with competence across the J-space ($\rho = 0.97$, $p< 10^{-4}$), the full activation space ($\rho = 0.93$, $p= 0.0001$), and the remainder ($\rho = 0.92$, $p= 0.0002$). We find that the J-space and full activation convergences are nearly identical. 
