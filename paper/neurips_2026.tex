...
We repeat the content-ablation experiment of Section~\ref{sec:textresults}, again only varying the metric. Results are shown in Table~\ref{tab:metriccontent}. Under the three global metrics, we find that full-space alignment is essentially unchanged by the ablation: each metric retains over $96.9\%$. For J-space ablation, the same metrics retain $88.1\%$, $85.4\%$, and $77.6\%$. In other words, at most a fifth of J-space alignment is attributable to semantic content. For neighbourhood-based measures, the opposite is true. 
