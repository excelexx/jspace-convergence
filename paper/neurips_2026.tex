...
\begin{figure}[t]
  ...
  \caption{\textbf{Measuring Platonic convergence in the J-space.}
    ...
    example found in Section~3.1. The list of words is illustrative.
    \textbf{(b)} Randomising content words retains $26.3\%$ of J-space
    alignment and $54.1\%$ of full-activation alignment. Under the same measurement with a random Gaussian dictionary rather than a J-lens dictionary, alignment retains $34.7\%$.
...
Thus, we hypothesise that restricting the alignment comparison to J-spaces will yield a more semantically selective measure of convergence than the full-activation comparison, in the sense that J-space alignment will drop more sharply when semantic content is removed (Figure~\ref{fig:teaser}). 
...
We note that within language, regardless of metric, full alignment is always greater than J-space alignment. Cross-modally the two are near-indistinguishable, with J-space marginally ahead for five of the eight metrics, while the cross-modal correlation result is similarly strong ($+0.94 \leq \rho \leq +0.97$).
