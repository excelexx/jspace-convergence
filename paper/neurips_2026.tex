...
Whether these alignment scores place weight onto semantic content, however, is less settled. Furthermore, previous work varies the \emph{metric}, while always comparing the full activation space. In contrast, we hold the metric fixed and restrict the comparison to a specific \emph{subspace}. 
...
\item We show that J-space convergence is meaningful and can be read as words using the J-lens. At matched relative layer depth, independently trained models name 5.1 of 25 shared vocabulary words against 2.0 for the plain logit lens. J-lens readouts also converge as competence increases, at least as closely as the kernel measure.
\end{enumerate}
...
First, we test only 11 text models and four vision encoders. The text models span 70M--4.3B parameters, and all four vision encoders
